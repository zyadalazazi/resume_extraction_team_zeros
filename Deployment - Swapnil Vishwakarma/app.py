...
test_data = pickle.load(open('testFile.text', 'rb'))
...
def resumeData(text,model):
    ...
    company = [ee for ee in doc.ents if ee.label_ == 'COMPANIES WORKED AT']
    name = [ee for ee in doc.ents if ee.label_ == 'NAME']
    designation = [ee for ee in doc.ents if ee.label_ == 'DESIGNATION']
    degree = [ee for ee in doc.ents if ee.label_ == 'DEGREE']
    graduationYear = [ee for ee in doc.ents if ee.label_ == 'GRADUATION YEAR']
    skills = [ee for ee in doc.ents if ee.label_ == 'SKILLS']
    collegeName = [ee for ee in doc.ents if ee.label_ == 'COLLEGE NAME']
    location = [ee for ee in doc.ents if ee.label_ == 'LOCATION']
    email = [ee for ee in doc.ents if ee.label_ == 'EMAIL ADDRESS']
